# N = 500, 1000, 2000, 3000, 4000, 5000, 10000, 15000, 20000
...
def main():
    ...
    def create_model_input_(nodes, object_pose, label):
        bfk = BatchFk(
            urdf_path="resources/robots/stretch/stretch.urdf", 
            ee_link_name='camera_color_optical_frame_rotated', 
            device='cuda', 
            batch_size=nodes.shape[0]
        )

        _, _, camera_matrices = bfk.batch_fk(nodes)
        model_input_tensor = torch.zeros((bfk.batch_size, 10), dtype=torch.float32).to(torch.device("cuda"))
        object_matrix = bfk.pose_to_transformation_matrix(object_pose)
        pose_list_tensor = bfk.transform_poses_batch(camera_matrices, object_matrix)
    ...
    def calculate_trajectory_perc_score(trajectory, object_pose, object_label, model):
        ...
        model_input = create_model_input_(trajectory, object_pose, object_label)
        # print(model_input)
    ...
    for obj in objects:  # human, monitor, cup
        ...
        for start in starts:
            for goal in goals:
                times = []
                for i in range(50):  # 50 trials per start/goal pair, 
                    # Build PRM and add start and goal
                    seed = SEED + i  # Different seed for each trial
                    trajectory, time_ = generate_trajectory(env, MODEL, seed, start, goal)
                    times.append(time_)
                ...
                whislo = np.min(times[times >= lower_bound])  # Lowest value within bounds
                whishi = np.max(times[times <= upper_bound])  # Highest value within bounds
                ...
                boxplot_stats = {
                    ...
                    'q3': q3,
                    'whishi': whishi,
                    'fliers': times[(times < lower_bound) | (times > upper_bound)].tolist()
                }
    ...
    # 100, 500, 1000, 2500, 5000, 10000, 15000, 20000
    N = 100

    with open(f'./exp_results/comparison_experiment_trajectories_{N}.json', 'w') as f:
