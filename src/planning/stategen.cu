...
namespace prm::construction{
    ...
    __global__ void generateRandomVectors(float* states, unsigned long seed, int numStates) {
        ...
        states[base + 0] = LOWER_BOUNDS[0] + (UPPER_BOUNDS[0]-LOWER_BOUNDS[0]) * curand_uniform(&state); // Random [0,1)
...
//     generateRandomVectors<<<gridsize, blocksize>>>(d_poses, seed, numPoses);
